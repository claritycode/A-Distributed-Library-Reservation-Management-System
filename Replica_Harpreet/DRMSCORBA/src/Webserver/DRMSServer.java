...
public class DRMSServer extends DRMSServices.LibraryInterfacePOA implements Runnable 
{
	...
	public DRMSServer(int portNumber,String name)
	{
		try
		{
	...
	//--------------------------------------------------Thread Function Run---------------------------------------------------
		...
		public void run()
		{
			try
			{
				...
				while(true)				// while loop So That Server is continuously Listening at its Server Port for the connection request from other Servers
				{
				    ...
				    server.start();
				    System.out.println("the value of the i in the concordia server"+i);
				}
				}
				catch(Exception e)
				{
		...
		public static void main(String args[])
		{
			try
			{
				System.out.println("Enter the port at which u want to start the concordia server");
				int portConcordia=Integer.parseInt(new BufferedReader(new InputStreamReader(System.in)).readLine());
				System.out.println("Enter the name of the Server u want to start");
				String name=(new BufferedReader(new InputStreamReader(System.in)).readLine());
				DRMSServer concordiaServer=new DRMSServer(portConcordia,name);//Creating the object of the Server So as to initate the server and create threads on that server 
	...
	{
		System.out.println("in the create account of Concordia Server");
		...
		case 1:			{
						...
						*/
						...
						synchronized(hash.get(key))
						{
							...
							java.util.Collections.sort(hash.get(key),new StudentArrayListSorter());
							log("Account credated For concordia :"+_username+" "+_institute+" "+_phone_no);
							System.out.println("Account credated For :"+_username+" "+_institute+" "+_phone_no);
	...
	{
		System .out.println("In the Reserve book funt. of the Concordia server");
		if(studentLogin(_userName,password))
		{
		Character _keyStudent=_userName.charAt(0);
		Character _keyBook=_bookName.charAt(0);
		System .out.println("In the Reserve book funt. of the Concordia server   "+_bookName+"--"+_keyStudent+"--"+_keyBook);
		...
		System.out.println(_bookExists.size()+"--------------");//_bookExists.get(0).m_bookName);
...
//---------------------------Add Book Function For ADMIN-------------------
	...
	public boolean addBook(String _bookName,String _authorName,int _noOfcopies)
	{
		...
		System.out.println("in the add function of the Concordia server for thr book"+_bookName);
...
*/
	...
	{
		System.out.println("In the Get Non Returners function of the concordia Server");
...
*/
	...
	public String getNonReturnersLocal(int duration)
	{
		...
		System.out.println("Im the local function of get Non Returnersof Concordia Server");
...
*/
	...
	*/
	...
	{
			System.out.println("In the Inter Library function of concordia");
							...
							try
							{
								...
								else
								{
									...
									if(new String(reply.getData()).trim().equals("truee"))
									{
											...
											log("The Book "+_bookname+"Found at                the Sherbrooke Library");
											atomicity=reservedBookInRemoteLibrary(_username,_bookname);
											acknowledgement=new String("acknowledgement "+_bookname+" "+Boolean.toString(atomicity));
											b=acknowledgement.getBytes();
											request=new DatagramPacket(b,b.length,ip,serverPort);
