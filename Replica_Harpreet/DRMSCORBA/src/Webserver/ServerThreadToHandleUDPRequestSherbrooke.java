...
public class ServerThreadToHandleUDPRequestSherbrooke extends Thread
{
	DRMSSherbrooke server;   // The object of the  Server is made so as to invoke the UDP Functions on it 
	...
	*/
	 public ServerThreadToHandleUDPRequestSherbrooke(DRMSSherbrooke server, DatagramPacket request)
	{
		this.server=server;
		...
		//ServerObject.getNonReturners();
		 ...
		 if(new String(request.getData()).contains(" "))
			 
		    {
			 	...
			 	{
			 		System.out.println("in the thread of the sherbrooke Server");
				 	server.bookDetailsPrint();
		    	...
		    	server.log(reserveInterLibraryString[1]+"  Trying to reserve the book  "+reserveInterLibraryString[2]+"   "+reserveInterLibraryString[3]+"-----  "+_wasBookReserved+" "+Calendar.getInstance().getTimeInMillis() );
		    	byte _wasBookReservedReply[]=new byte[5000];
		    	_wasBookReservedReply=(Boolean.toString(_wasBookReserved)).getBytes();
		    	System.out.println("-------------------------------------Byte Value"+Boolean.toString(_wasBookReserved));
		    	...
		    	server.log("The Reply has been sent back to the server");
			 	...
			 	{
			 		server.log("the Acknowledgement recieved "+reserveInterLibraryString[2].trim());
		    ...
		    String _instituteName="Sherbrooke";
		    
					

			    byte[] nonreturenrsByte ;
...
*/
			    	...
			    	nonreturenrsByte= new byte[nonreturners.length()];
