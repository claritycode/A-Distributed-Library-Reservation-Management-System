...
{
	DRMSServerInterface _serverConcordia,_serverSherbrooke,_serverMcgill;// the Object of the Concordia Server
	//DRMSSherbrooke _serverSherbrooke;// the Object Of the Sherbrooke Server
	//DRMSMcgill _serverMcgill; // the Object of the Mcgill server
	...
	public DRMSStudent() 
	{
		...
		try
		{
			...
			URL urlConcordia =new URL("http://localhost:8080/webConcordia?wsdl");//creating theURLfor the server 
			System.out.println("the URL is created "+urlConcordia);
			URL urlSherbrooke =new URL("http://localhost:8082/webSherbrooke?wsdl");//creating theURLfor the server 
			URL urlMcgill =new URL("http://localhost:8081/webMcgill?wsdl");//creating theURLfor the server 
			
			
			
			
			QName qnameConcordia= new QName("http://Webserver/","DRMSServerService");//creating the Qname for the Server
			QName qnameSherbrooke= new QName("http://Webserver/","DRMSSherbrookeService");//creating the Qname for the Server
			QName qnameMcgill= new QName("http://Webserver/","DRMSMcgillService");//creating the Qname for the Server
			
			
			Service serviceConcordia= Service.create(urlConcordia,qnameConcordia);//Creating the Service that is binding the URL With the Qname
			Service serviceSherbrooke= Service.create(urlSherbrooke,qnameSherbrooke);//Creating the Service that is binding the URL With the Qname
			Service serviceMcgill= Service.create(urlMcgill,qnameMcgill);//Creating the Service that is binding the URL With the Qname
			_serverConcordia=serviceConcordia.getPort(DRMSServerInterface.class);//Creating the object of the server and intializing it with the value of the Srvice PORT
			_serverMcgill=serviceMcgill.getPort(DRMSServerInterface.class);//Creating the object of the server and intializing it with the value of the Srvice PORT
			_serverSherbrooke=serviceSherbrooke.getPort(DRMSServerInterface.class);//Creating the object of the server and intializing it with the value of the Srvice PORT
	...
	//---------------------run function of the thread to create account of the students to the specific server-------------------------------------------------------------------
		public void run ()
		{
			try
			{
				...
				_serverConcordia.createAccount("harpreet","harpreet","harpreetEmail","51444","harpreet","harpreet","concordia");
				_serverMcgill.createAccount("gurpreet","singh","harpreetEmail","54441","harpreet","harpreet","mcgill");
				_serverSherbrooke.createAccount("harmanpreet","singh","harpreetEmail","55544","harpreet","harpreet","sherbrooke");
				_serverConcordia.addBook("book1","author1",4);
				_serverMcgill.addBook("book2", "author2",4);
				_serverSherbrooke.addBook("book3", "author3",4);
			}
			catch(Exception e)
			{
				e.printStackTrace();
		...
		*/
		public void studentCreateAccount()
		{
			...
			System.out.println("please enter Your Email:");//Reading the Email for the User From The console
			_email=br.readLine();	
			System.out.println("please enter Your Phone Number:");//Reading the Phone Number for the User From The console
			_phone_no=(br.readLine());
			...
			if(_institute.equalsIgnoreCase("concordia"))
			{
				...
				//DRMSServerInterface Server=(DRMSServerInterface)Naming.lookup("rmi://localhost:5000/concordia");
				_checkBoolean=_serverConcordia.createAccount(_first_name,_last_name,_email,_phone_no,_username,_password,_institute);
			...
			{
				_checkBoolean=_serverMcgill.createAccount(_first_name,_last_name,_email,_phone_no,_username,_password,_institute);
			...
			{
				_checkBoolean=_serverSherbrooke.createAccount(_first_name,_last_name,_email,_phone_no,_username,_password,_institute);
			...
			{
				System.out.println("The Account has not been creadted");
			...
			try
			{
				...
				case 1:
						...
						while(_continueFlag==false)
						{
							...
							if(_institute.equalsIgnoreCase("concordia"))
							_continueFlag=_serverConcordia.reserveBook(_userName,_password,_bookName,_authorName);//Calling the reserveBook function on the Concordia server Object
							else if(_institute.equalsIgnoreCase("mcgill"))
							_continueFlag=_serverMcgill.reserveBook(_userName,_password,_bookName,_authorName);	//Calling the reserveBook function on the Mcgill server Object
							else if(_institute.equalsIgnoreCase("sherbrooke"))
								_continueFlag=_serverSherbrooke.reserveBook(_userName,_password,_bookName,_authorName);//Calling the reserveBook function on the Sherbrooke server Object
							if(_continueFlag)
							{
...
//-----------------------------------------------------Main Class--------------------------------------------------------
		...
		public static void main(String[] args)
		{
			...
			System.out.println("The object of Student Class created");
			BufferedReader br= new BufferedReader(new InputStreamReader(System.in));
									...
									while(_continueFlag==false)
									{
										...
										switch(choice)
										{
											case 1:
															...
															_noOfCopies=Integer.parseInt(br.readLine());
															
															/*System.out.println("enter the Author name of the book");
															_authorName=br.readLine();*/
															if(_institute.equalsIgnoreCase("concordia"))
															{
															_continueFlag=_serverConcordia.addBook(_bookName,_authorName,_noOfCopies);
															System.out.println(_continueFlag+"  The value of the continue flag");
															if(_continueFlag)
															log(_userName,"add the book at concordia "+_bookName+" "+_authorName);
															}
															else if(_institute.equalsIgnoreCase("mcgill"))
															{
																_continueFlag=_serverSherbrooke.addBook(_bookName,_authorName,_noOfCopies);
																if(_continueFlag)
															...
															if(_continueFlag)
																...
																log(_userName,"Book Has Been Added");
																}
															else
															{
																System.out.println("The Specified Book Cannot be Located");
											...
											case 3:				System.out.println("Enter the name of the student");
																...
																while(valid)
																	...
																	try
																	{
																		...
																		setDuration(student,bookName,_numberOfDays,_institute);
																		_continueFlag=true;
																		valid=false;
																	}
																	catch(Exception e)
...
//--------------------------------------------function to set duration for the specific student ---------------------------------------
					public void setDuration(String student,String bookName,int _mNoOfDays,String _institute )
					{
						try
						{
						boolean _continueFlag=false;
						if(_institute.equalsIgnoreCase("concordia"))
							_continueFlag=_serverConcordia.setDuration(student,bookName,_mNoOfDays);
							else if(_institute.equalsIgnoreCase("mcgill"))
							_continueFlag=_serverMcgill.setDuration(student,bookName,_mNoOfDays);
							else if(_institute.equalsIgnoreCase("sherbrooke"))
								_continueFlag=_serverSherbrooke.setDuration(student,bookName,_mNoOfDays);
							if(_continueFlag)
								{
								System.out.println(_continueFlag+"------");
